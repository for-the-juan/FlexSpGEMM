...
void MultiplyspECKImplementation(const dCSR<DataType> &matA_Dealloc, const dCSR<DataType> &matB_Dealloc,
		dCSR<DataType> &matOut, spECKConfig &config, Timings &timings)
{
    ...
    // printf("the entriesPerWarpCounting is %d\n",entriesPerWarpCounting);
    ...
    // printf("the MAX_DYNAMIC_SHARED is %d\n",MAX_DYNAMIC_SHARED);
    ...
    // calc amount of operations per row
    {
        ...
        // readOperations<threadsPerBlock><<<divup(uint32_t(matA.rows), rowsPerBlock), threadsPerBlock>>>(
        //     matA.row_offsets, matA.col_ids,matA.rows,matA.nnz,matB.row_offsets,matB.col_ids,matB.rows,matB.nnz, d_rowOperations, rowsPerBlock, d_maxComputationsPerRow, d_rowColMinMax, d_rowMaxOperations, d_sumProducts);
        // printf("the matB.col_ids[0] is %u\n",matB.col_ids[0]);
        // check_array_in_device_unit32<<<1,1>>>(2,matB.col_ids);
        // printf("the rowsPerBlock is %d, (uint32_t(matA.rows) is %u\n",rowsPerBlock,uint32_t(matA.rows));
        // printf("the rowsPerBlock is %d, divup(uint32_t(matA.rows), rowsPerBlock) is %d\n",rowsPerBlock,divup(uint32_t(matA.rows), rowsPerBlock));
        // printf("the threadsPerBlock is %d, entriesPerWarpCounting * warpsCounting is %d\n",threadsPerBlock,entriesPerWarpCounting * warpsCounting);
    ...
    {
        //printf("inside Multiply.cu:595 malloc C matOut.nnz %d matC.nnz %d\n", matOut.nnz, matC.nnz);
    ...
    // printf("the reprocessLoadBalanceNumeric is %d\n",reprocessLoadBalanceNumeric);
    ...
    if (isDenseOutput)
    {
        ...
        rowOffsetMapCount = min(h_blockCounterScaleNumeric[0], sm * BLOCKS_PER_SM);//BLOCKS_PER_SM=4
    ...
    bool sortAllInplace = false;
    {
        ...
        // -------------------------------------------------------------------------------------------------------------------------------------------
        #if compute_share
        unsigned long long practical_provide=0;
        unsigned long long true_need=0;
        true_need= (unsigned long long)(matC.nnz)*12;
        #endif
        // printf("matC.nnz is %lld\n",matC.nnz);
        // printf("true_need is %lld\n",true_need);
        // printf("dynamicSharedBytesPerBlockNumeric is %lld\n",dynamicSharedBytesPerBlockNumeric);
        // printf("sharedBytesPerBlockNumeric is %lld\n",sharedBytesPerBlockNumeric);
        // for(int i = 0;i<6;i++){
        //     printf("the h_blockCounterScaleNumeric[%d] is %d\n",i,h_blockCounterScaleNumeric[i]);
        // }
        if (h_blockCounterScaleNumeric[0] > 0)
        {
            #if compute_share
            practical_provide += (unsigned long long)h_blockCounterScaleNumeric[0]*(dynamicSharedBytesPerBlockNumeric >> 0);
            // printf("(unsigned long long)h_blockCounterScaleNumeric[0]*(dynamicSharedBytesPerBlockNumeric >> 0) is %lld\n",(unsigned long long)h_blockCounterScaleNumeric[0]*(dynamicSharedBytesPerBlockNumeric >> 0));
            #endif  
            spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[0], streams[0], 32 * warpsNumeric, dynamicSharedBytesPerBlockNumeric);
            // printf("the dynamicSharedBytesPerBlockNumeric is %d\n",dynamicSharedBytesPerBlockNumeric);
            spgemm.h_DenseSpGEMMNumeric<IndexType, GlobalMapRowOffsets, dynamicSharedBytesPerBlockNumeric, true, (32 * warpsNumeric)>(
                ...
                // practical_provide += (unsigned long long)h_blockCounterScaleNumeric[0]*dynamicSharedBytesPerBlockNumeric;
                // printf("h_blockCounterScaleNumeric[0]*(dynamicSharedBytesPerBlockNumeric) is %lld\n",h_blockCounterScaleNumeric[0]*(sharedBytesPerBlockNumeric));
                // printf("h_blockCounterScaleNumeric[0] is %d\n",h_blockCounterScaleNumeric[0]);
        ...
        {
            #if compute_share
                practical_provide += (unsigned long long)h_blockCounterScaleNumeric[1]*(sharedBytesPerBlockNumeric >> 0);
                // printf("h_blockCounterScaleNumeric[1]*(sharedBytesPerBlockNumeric >> 0) is %lld\n",h_blockCounterScaleNumeric[1]*(sharedBytesPerBlockNumeric >> 0));
                // printf("h_blockCounterScaleNumeric[1] is %d\n",h_blockCounterScaleNumeric[1]);
            #endif
            if (spGemmMethodNumeric == Config::AutoSpGEMM)
            {
                ...
                spgemm.h_SpGEMMNumericLauncher<IndexType, GlobalMap, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 0), false, (32 * warpsNumeric >> 0)>(
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[1], streams[1], 32 * warpsNumeric >> 0, (sharedBytesPerBlockNumeric >> 0));
                spgemm.h_DenseSpGEMMNumeric<IndexType, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 0), false, (32 * warpsNumeric >> 0)>(
            ...
            else
            {
                ...
                spgemm.h_HashSpGEMMNumeric<IndexType, GlobalMap, (sharedBytesPerBlockNumeric >> 0), false, (32 * warpsNumeric)>(
        ...
        {
            #if compute_share
            practical_provide += (unsigned long long)h_blockCounterScaleNumeric[2]*(sharedBytesPerBlockNumeric >> 1);
            // printf("h_blockCounterScaleNumeric[2]*(sharedBytesPerBlockNumeric >> 1) is %lld\n",h_blockCounterScaleNumeric[2]*(sharedBytesPerBlockNumeric >> 1));
            // printf("h_blockCounterScaleNumeric[2] is %d\n",h_blockCounterScaleNumeric[2]);
            #endif
            if (spGemmMethodNumeric == Config::AutoSpGEMM)
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[2], streams[2], (32 * warpsNumeric >> 1), (sharedBytesPerBlockNumeric >> 1));
                spgemm.h_SpGEMMNumericLauncher<IndexType, GlobalMap, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 1), false, (32 * warpsNumeric >> 1)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount, rowOffsetMaps, rowOffsetMapCount,
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[2], streams[2], 32 * warpsNumeric >> 1, (sharedBytesPerBlockNumeric >> 1));
                spgemm.h_DenseSpGEMMNumeric<IndexType, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 1), false, (32 * warpsNumeric >> 1)>(
                    matA, matB, matC, (GlobalMapRowOffsets *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[2], d_rowOperations,
                    h_blockCounterScaleNumeric[2], d_rowColMinMax,
                    d_rowMaxOperations, setSortingBit, rowsPerBlock);
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[2], streams[2], 32 * warpsNumeric >> 1, (sharedBytesPerBlockNumeric >> 1));
                spgemm.h_HashSpGEMMNumeric<IndexType, GlobalMap, (sharedBytesPerBlockNumeric >> 1), false, (32 * warpsNumeric >> 1)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[2], d_rowOperations,
                    sortMode,
                    h_blockCounterScaleNumeric[2], d_rowColMinMax,
        ...
        {
            #if compute_share
            practical_provide += (unsigned long long)h_blockCounterScaleNumeric[3]*(sharedBytesPerBlockNumeric >> 2);
            // printf("h_blockCounterScaleNumeric[3]*(sharedBytesPerBlockNumeric >> 2) is %lld\n",h_blockCounterScaleNumeric[3]*(sharedBytesPerBlockNumeric >> 2));
            // printf("h_blockCounterScaleNumeric[3] is %d\n",h_blockCounterScaleNumeric[3]);
            #endif
            if (spGemmMethodNumeric == Config::AutoSpGEMM)
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[3], streams[3], (32 * warpsNumeric >> 2), (sharedBytesPerBlockNumeric >> 2));
                spgemm.h_SpGEMMNumericLauncher<IndexType, GlobalMap, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 2), false, (32 * warpsNumeric >> 2)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount, rowOffsetMaps, rowOffsetMapCount,
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[3], streams[3], 32 * warpsNumeric >> 2, (sharedBytesPerBlockNumeric >> 2));
                spgemm.h_DenseSpGEMMNumeric<IndexType, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 2), false, (32 * warpsNumeric >> 2)>(
                    matA, matB, matC, (GlobalMapRowOffsets *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[3], d_rowOperations,
                    h_blockCounterScaleNumeric[3], d_rowColMinMax,
                    d_rowMaxOperations, false, rowsPerBlock);
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[3], streams[3], 32 * warpsNumeric >> 2, (sharedBytesPerBlockNumeric >> 2));
                spgemm.h_HashSpGEMMNumeric<IndexType, GlobalMap, (sharedBytesPerBlockNumeric >> 2), false, (32 * warpsNumeric >> 2)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[3], d_rowOperations,
                    sortMode,
                    h_blockCounterScaleNumeric[3], d_rowColMinMax,
        ...
        {
            #if compute_share
            practical_provide += (unsigned long long)h_blockCounterScaleNumeric[4]*(sharedBytesPerBlockNumeric >> 3);
            // printf("h_blockCounterScaleNumeric[4]*(sharedBytesPerBlockNumeric >> 3) is %lld\n",h_blockCounterScaleNumeric[4]*(sharedBytesPerBlockNumeric >> 3));
            // printf("h_blockCounterScaleNumeric[4] is %d\n",h_blockCounterScaleNumeric[4]);
            #endif
            if (spGemmMethodNumeric == Config::AutoSpGEMM)
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[4], streams[4], (32 * warpsNumeric >> 3), (sharedBytesPerBlockNumeric >> 3));
                spgemm.h_SpGEMMNumericLauncher<IndexType, GlobalMap, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 3), false, (32 * warpsNumeric >> 3)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount, rowOffsetMaps, rowOffsetMapCount,
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[4], streams[4], 32 * warpsNumeric >> 3, (sharedBytesPerBlockNumeric >> 3));
                spgemm.h_DenseSpGEMMNumeric<IndexType, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 3), false, (32 * warpsNumeric >> 3)>(
                    matA, matB, matC, (GlobalMapRowOffsets *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[4], d_rowOperations,
                    h_blockCounterScaleNumeric[4], d_rowColMinMax,
                    d_rowMaxOperations, false, rowsPerBlock);
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[4], streams[4], 32 * warpsNumeric >> 3, (sharedBytesPerBlockNumeric >> 3));
                spgemm.h_HashSpGEMMNumeric<IndexType, GlobalMap, (sharedBytesPerBlockNumeric >> 3), false, (32 * warpsNumeric >> 3)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[4], d_rowOperations,
                    sortMode,
                    h_blockCounterScaleNumeric[4], d_rowColMinMax,
        ...
        {
            #if compute_share
            practical_provide += (unsigned long long)h_blockCounterScaleNumeric[5]*(sharedBytesPerBlockNumeric >> 4);
            // printf("the h_blockCounterScaleNumeric[5]*(sharedBytesPerBlockNumeric >> 4) is lld%\n",h_blockCounterScaleNumeric[5]*(sharedBytesPerBlockNumeric >> 4));
            // printf("the h_blockCounterScaleNumeric[5] is %d\n",h_blockCounterScaleNumeric[5]);
            #endif
            if (spGemmMethodNumeric == Config::AutoSpGEMM || ((rowsPerBlock > 1 || reprocessLoadBalanceNumeric) && spGemmMethodNumeric != Config::HashSpGEMM))
            {
                ...
                spgemm.h_SpGEMMNumericLauncher<IndexType, GlobalMap, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 4), false, (32 * warpsNumeric >> 4)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount, rowOffsetMaps, rowOffsetMapCount,
            ...
            else if (spGemmMethodNumeric == Config::DenseSpGEMM)
            {
                ...
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[5], streams[5], 32 * warpsNumeric >> 4, (sharedBytesPerBlockNumeric >> 4));
                spgemm.h_DenseSpGEMMNumeric<IndexType, GlobalMapRowOffsets, (sharedBytesPerBlockNumeric >> 4), false, (32 * warpsNumeric >> 4)>(
                    matA, matB, matC, (GlobalMapRowOffsets *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[5], d_rowOperations,
                    h_blockCounterScaleNumeric[5], d_rowColMinMax,
                    d_rowMaxOperations, false, rowsPerBlock);
            ...
            {
                spgemm.setLaunchDimensions(h_blockCounterScaleNumeric[5], streams[5], 32 * warpsNumeric >> 4, (sharedBytesPerBlockNumeric >> 4));
                spgemm.h_HashSpGEMMNumeric<IndexType, GlobalMap, (sharedBytesPerBlockNumeric >> 4), false, (32 * warpsNumeric >> 4)>(
                    matA, matB, matC, (GlobalMap *)hashMaps, hashMapCount,
                    d_blockStartRows + blockPrefixScaled[5], d_rowOperations,
                    sortMode,
                    h_blockCounterScaleNumeric[5], d_rowColMinMax,
    ...
    #if compute_share
        double use_of_shared = (double)((double)true_need/(double)practical_provide);
        // printf("the practical_provide is %lld\n",practical_provide);
        // printf("the use_of_shared is %lf\n",use_of_shared);
        FILE *fout_mem = fopen("/home/wm/open_source_code/spGEMM/OpSparse-main/spECK/data/spECK_new_repres_use_of_shared.csv", "a");
        if (fout_mem == NULL)
            printf("Writing results fails.\n");
        fprintf(fout_mem, "%i,%f\n",
            matA.rows,use_of_shared);
        fclose(fout_mem);
    #endif
    }
    ...
    if (!sortAllInplace && (h_blockCounterScaleNumeric[1] + h_blockCounterScaleNumeric[2] > 0) && maxElementsPerRow >= 500)
    {
        if (h_blockCounterScaleNumeric[2] > 0)
        {
            ...
            spgemm.h_HashSpGEMMSorting<uint32_t, 32 * warpsNumeric / 4, entriesPerWarpNumeric * 32 / 2>(
        ...
        if (h_blockCounterScaleNumeric[1] > 0)
        {
            ...
            spgemm.h_HashSpGEMMSorting<uint32_t, 32 * warpsNumeric / 2, entriesPerWarpNumeric * 32>(
...
{
	MultiplyspECKImplementation<double, BLOCKS_PER_SM, THREADS_PER_BLOCK, MAX_DYNAMIC_SHARED, MAX_STATIC_SHARED>(A, B, matOut, config, timings);
}
