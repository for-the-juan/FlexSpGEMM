...
void MultiplyspECKImplementation(const dCSR<DataType> &matA_Dealloc, const dCSR<DataType> &matB_Dealloc, dCSR<DataType> &matOut, spECKConfig &config, Timings &timings)
{
    ...
    // printf("i getin MultiplyspECKImplementation\n ");  // 调试用，benchmark 时关闭
    if (matB.cols > 1 << 27)
    {
    ...
    // printf("matB.nnz is %u\n",matB.nnz);
    // printf("matA.nnz is %u\n",matA.nnz);
    if (matA.nnz * matB.nnz == 0) {
        printf("matA.nnz * matB.nnz == 0\n");
        matOut.nnz = 0;
        return;
    }
    ...
    // calc amount of operations per row
    {
        ...
        // 核函数启动失败
            printf("readOperations  failed\n");
        // 处理失败情况
        } else {
        // 核函数启动成功
            // printf("readOperations success,\n");
        // 执行后续操作
        }
        check_array_in_device_unit32<<<1,1>>>(2,matA.col_ids);
        check_array_in_device_unit32<<<1,1>>>(2,matA.row_offsets);
        // check_array_in_device_unit32<<<1,1>>>(2,matA.col_ids);
        // check_array_in_device_unit32<<<1,1>>>(2,matA.col_ids);
        ...
        if (cudaStatus != cudaSuccess) {
        // 核函数启动失败
            printf("check_array_in_device_unit32  failed\n");
        // 处理失败情况
        } else {
        // 核函数启动成功
            // printf("check_array_in_device_unit32 success,\n");
        // 执行后续操作
        }
        uint32_t tmpArr[2];
    ...
    // printf("i getin useLoadBalancingCounting\n ");
