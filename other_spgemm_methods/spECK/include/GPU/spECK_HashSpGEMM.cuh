...
template <typename INDEX_TYPE, uint32_t MAP_SIZE, uint32_t THREADS, bool DIRECT, uint32_t SHIFT, class LocalMap>
__device__ void iterateMatrixNumeric(INDEX_TYPE startId, INDEX_TYPE lastIdExcl, INDEX_TYPE startRow, INDEX_TYPE lastRowExcl,
	const INDEX_TYPE* __restrict__ rowOffsetsA, const INDEX_TYPE* __restrict__ rowOffsetsB,
	const INDEX_TYPE* __restrict__ colIdsA, const INDEX_TYPE* __restrict__ colIdsB,
	const double* __restrict__ valuesA, const double* __restrict__ valuesB,
	const INDEX_TYPE nnzA, const INDEX_TYPE nnzB, const INDEX_TYPE rowsA, const INDEX_TYPE rowsB,
...
// this class creates all instances of my iterateMatrix template and selects the best instance for the specific call 'call'
template <typename INDEX_TYPE, uint32_t MAP_SIZE, uint32_t THREADS, class LocalMap>
...
#define iterate_templ_Numeric(startId, lastIdExcl, shift, direct) iterateMatrixNumeric<INDEX_TYPE, MAP_SIZE, THREADS, direct, shift>( \
	startId, lastIdExcl, startRow, lastRowExcl, \
	rowOffsetsA, rowOffsetsB, colIdsA, colIdsB, valuesA, valuesB, \
	nnzA, nnzB, rowsA, rowsB,\
	lMap, resultNnz, minCol);
...
#define iterate_templ_Numeric(startId, lastIdExcl, shift, direct) iterateMatrixNumericSingleRow<INDEX_TYPE, THREADS, direct, shift>( \
	startId, lastIdExcl,                                                                                                                             \
	rowOffsetsB, colIdsA, colIdsB, valuesA, valuesB,                                                                                                          \
	nnzB, rowsB,                                                                                                                                              \
	lMap, resultNnz, minCol);
...
__device__ __forceinline__ void directSpGEMMNumericImplementation(
	...
	const double* __restrict__ valuesA, const double* __restrict__ valuesB,
	INDEX_TYPE* __restrict__ colIdsC, double* __restrict__ valuesC,
	...
	uint32_t rowsPerBlock)
{
	...
	typedef HashMap<INDEX_TYPE, double> LocalMap;
	const int mapSize = SHARED_HASH_SIZE / (sizeof(INDEX_TYPE) + sizeof(double));
	const uint32_t elementsPerThread = (mapSize + THREADS - 1) / THREADS > 0 ? (mapSize + THREADS - 1) / THREADS : 1;

	typedef IterateMatrixNumericMethods<INDEX_TYPE, mapSize, THREADS, LocalMap> IterateMatrix;
	typedef cub::BlockRadixSort<INDEX_TYPE, THREADS, elementsPerThread, double> RadixSort;
	...
	lMap.values = (double *)&dynamicMem[0];
	...
	#pragma unroll
		for (uint32_t i = 0; i < mapSize; i += THREADS)
		{
			...
			double val = valid ? lMap.values[index] : 0;
	...
	bool setSortedBit, uint32_t rowsPerBlock)
{
	...
	typedef IterateMatrixNumericSingleRowMethods<INDEX_TYPE, THREADS, LocalMap> IterateMatrix;
	typedef cub::BlockRadixSort<INDEX_TYPE, THREADS, elementsPerThread, double> RadixSort;
...
hashSpGEMMNumeric(
	...
	double *__restrict__ matCValues, const INDEX_TYPE *__restrict__ rowOffsetsC, INDEX_TYPE matCNnz, INDEX_TYPE *__restrict__ blockStartRow,
	const INDEX_TYPE *__restrict__ rowOperations,
	...
	uint32_t *rowMaxOperations, bool setSortedBit, uint32_t rowsPerBlock)
{
	...
	if (lastRowExcl - startRow > 1 || SUPPORT_GLOBAL) {
		hashSpGEMMNumericImplementation<INDEX_TYPE, SHARED_HASH_SIZE, THREADS>(
		...
		hashSpGEMMNumericSingleRowImplementation<INDEX_TYPE, SHARED_HASH_SIZE, THREADS>(
			nnzB, rowsB, 
			rowOffsetsB, colIdsA, colIdsB, valuesA, valuesB,
			matCColIds, matCValues, rowOperations, sortColumns,
...
template <typename INDEX_TYPE, uint32_t MAX_ELEMENTS_BLOCK, bool SUPPORT_GLOBAL, uint32_t THREADS, uint32_t SHIFT, bool useRowOffsets>
__device__ __forceinline__ void iterateMatrixDenseNumeric(
	...
	const INDEX_TYPE* __restrict__ rowOffsetsB,
	const double* __restrict__ valuesA, const double* __restrict__ valuesB,
	INDEX_TYPE* __restrict__ colIdsC, double* __restrict__ valuesC, const INDEX_TYPE minCol, const INDEX_TYPE maxCol,
	INDEX_TYPE* prefix, INDEX_TYPE* prefix2, 
	INDEX_TYPE* currentRowMinOffset, double* values, 
	void *temp_storage)
{
	...
	// 	printf("the spECK::numeric_limits<INDEX_TYPE>::max() is %lld\n",(long long)spECK::numeric_limits<INDEX_TYPE>::max());
	// }
	while (colOffset < maxCol)
	{
		...
		if (useRowOffsets) {
			for (INDEX_TYPE _idA = startId; _idA < lastIdExcl; _idA += THREADS >> SHIFT)
			{
				...
				const double valueA = valid ? valuesA[idA] : 0;
			...
			for (INDEX_TYPE idA = startId + (threadIdx.x >> SHIFT); idA < lastIdExcl; idA += THREADS >> SHIFT)
			{
				...
				const double valueA = valuesA[idA];
...
template <typename INDEX_TYPE,class GlobalRowOffsetsMap, uint32_t SHARED_MEM_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
	...
	uint32_t rowsPerBlock)
{
	...
	const uint32_t bitsPerElement = sizeof(double) * 8 + 2;
	...
	if (useGlobalOffsetsMap)
	{
		if(threadIdx.x == 0)
		{
			// printf("i use the useGlobalOffsetsMap---------------------------------\n");
		...
		values = (double*) &((char*)dynamicShared)[sizeof(BlockScan::TempStorage)];
	...
	else
	{
		...
		temp_storage = (typename  BlockScan::TempStorage*) (void*)dynamicShared;
		values = (double*) &((char*)dynamicShared)[sizeof(BlockScan::TempStorage)];
		// if(threadIdx.x==0 && startRow==1){
		// 	printf("the elementsPerBlock is %d\n",elementsPerBlock);
		// }
		prefix = (INDEX_TYPE*) &values[elementsPerBlock];
		prefix2 = (INDEX_TYPE*) &prefix[elementsPerBlock / 32 + 1];
...
#define iterate(SHIFT, useRowOffsets) iterateMatrixDenseNumeric<INDEX_TYPE, maxElementsBlock, SUPPORT_GLOBAL, THREADS, SHIFT, useRowOffsets>\
	...
	// printf("i get into globalOffsetsMap1\n");
	if(sMem.globalOffsetsMap != nullptr)
	{
		...
		if(threadIdx.x == 0)
		{
			// printf("i get into globalOffsetsMap2\n");
...
template <typename INDEX_TYPE,  class GlobalOffsetMap, uint32_t SHARED_HASH_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
__global__ void __launch_bounds__(1024, 1) denseSpGEMMNumeric(
	...
	double *__restrict__ valuesC, const INDEX_TYPE *__restrict__ rowOffsetsC, INDEX_TYPE matCNnz, INDEX_TYPE *__restrict__ blockStartRow,
	const INDEX_TYPE *__restrict__ rowOperations,
	...
	uint32_t *rowMaxOperations, bool setSortedBit, uint32_t rowsPerBlock)
{
	...
	denseSpGEMMNumericImplementation<INDEX_TYPE, GlobalOffsetMap, SHARED_HASH_SIZE, SUPPORT_GLOBAL, THREADS>(
...
template <typename INDEX_TYPE, class GlobalHashMap, class GlobalRowOffsetMap, uint32_t SHARED_MEM_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
__global__ void 
// __launch_bounds__(1024, 2) 
	...
	uint32_t *rowMaxOperations, uint32_t minimumDensity, bool setSortedBit, uint32_t rowsPerBlock)
{
	...
	const int hashMapSize = SHARED_MEM_SIZE / (sizeof(INDEX_TYPE) + sizeof(double)) - THREADS;
	...
	{
		directSpGEMMNumericImplementation<INDEX_TYPE, THREADS>(
	...
	{
		denseSpGEMMNumericImplementation<INDEX_TYPE, GlobalRowOffsetMap, SHARED_MEM_SIZE, SUPPORT_GLOBAL, THREADS>(
			nnzB, rowsB, rowOffsetsB, colIdsA, colIdsB, valuesA, valuesB,
	...
	if (lastRowExcl - startRow == 1 && !SUPPORT_GLOBAL) 
	{
		hashSpGEMMNumericSingleRowImplementation<INDEX_TYPE, SHARED_MEM_SIZE, THREADS>(
			nnzB, rowsB, 
			rowOffsetsB, colIdsA, colIdsB, valuesA, valuesB,
	...
	{
		hashSpGEMMNumericImplementation<INDEX_TYPE, SHARED_MEM_SIZE, THREADS>(
			nnzA, nnzB, rowsA, rowsB, rowOffsetsA, rowOffsetsB, colIdsA, colIdsB, valuesA, valuesB, 
			colIdsC, valuesC, rowOperations, sortColumns,
...
__global__ void hashSpGEMMSortingKernel(dCSRNoDealloc<double> matC, INDEX_TYPE *blockStartRow, uint32_t numberBlocks, bool bitShiftNumRows)
{
	const uint16_t elementsPerThread = (entriesPerBlock + THREADS - 1) / THREADS;
	typedef cub::BlockRadixSort<INDEX_TYPE, THREADS, elementsPerThread, double> BlockRadixSort;
	typedef HashMap<INDEX_TYPE, double> Map;

	__shared__ typename BlockRadixSort::TempStorage tempStorage;

	INDEX_TYPE threadIndices[elementsPerThread];
	double threadValues[elementsPerThread];
...
template <typename INDEX_TYPE, uint32_t THREADS, uint32_t entriesPerBlock>
void spECKKernels::h_HashSpGEMMSorting(dCSRNoDealloc<double> matC, INDEX_TYPE *blockStartRow, uint32_t numberBlocks, bool bitShiftNumRows)
{
	hashSpGEMMSortingKernel<INDEX_TYPE, THREADS, entriesPerBlock> << <gridDim, blockDim, sharedMem, stream >> > (matC, blockStartRow, numberBlocks, bitShiftNumRows);
}
...
template <typename INDEX_TYPE, class GlobalMap, uint32_t SHARED_HASH_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
void spECKKernels::h_HashSpGEMMNumeric(dCSRNoDealloc<double> matA, dCSRNoDealloc<double> matB, dCSRNoDealloc<double> matC, GlobalMap *maps, INDEX_TYPE mapCount,
...
{
	cudaFuncSetAttribute(hashSpGEMMNumeric<INDEX_TYPE, GlobalMap, SHARED_HASH_SIZE, SUPPORT_GLOBAL, THREADS>, 
		cudaFuncAttributeMaxDynamicSharedMemorySize, sharedMem);
	hashSpGEMMNumeric<INDEX_TYPE, GlobalMap, SHARED_HASH_SIZE, SUPPORT_GLOBAL, THREADS><<<gridDim, blockDim, sharedMem, stream>>>(
...
template <typename INDEX_TYPE, class GlobalMap, uint32_t SHARED_HASH_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
void spECKKernels::h_DenseSpGEMMNumeric(dCSRNoDealloc<double> matA, dCSRNoDealloc<double> matB, dCSRNoDealloc<double> matC, GlobalMap *maps, INDEX_TYPE mapCount,
	INDEX_TYPE *blockStartRow, INDEX_TYPE *rowOperations, uint32_t numberBlocks, const INDEX_TYPE* rowColMinMax,
	INDEX_TYPE *rowMaxOperations, bool setSortedBit, uint32_t rowsPerBlock)
{
	cudaFuncSetAttribute(denseSpGEMMNumeric<INDEX_TYPE, GlobalMap, SHARED_HASH_SIZE, SUPPORT_GLOBAL, THREADS>, 
		cudaFuncAttributeMaxDynamicSharedMemorySize, sharedMem);
	denseSpGEMMNumeric<INDEX_TYPE, GlobalMap, SHARED_HASH_SIZE, SUPPORT_GLOBAL, THREADS><<<gridDim, blockDim, sharedMem, stream>>>(
		matA.nnz, matB.nnz, matA.rows, matB.rows, matB.cols, matA.row_offsets, matB.row_offsets, matA.col_ids, matB.col_ids, matA.data, matB.data,
		maps, mapCount,
        ...
        if (error != cudaSuccess) {      
            printf("denseSpGEMMNumeric is failed\n");
        }else{
            // printf("denseSpGEMMNumeric is cudaSuccess\n");
        }
		// printf("for denseSpGEMMNumeric ,the gridDim is %d, blockDim is%d,sharedMem is %d\n",gridDim,blockDim,sharedMem);
}
...
void spECKKernels::h_SpGEMMNumericLauncher(dCSRNoDealloc<double> matA, dCSRNoDealloc<double> matB, dCSRNoDealloc<double> matC,
	GlobalHashMap *hashMaps, INDEX_TYPE hashMapCount, GlobalRowOffsetMap *rowOffsetMaps, INDEX_TYPE rowOffsetMapCount,
	INDEX_TYPE *blockStartRow, INDEX_TYPE *rowOperations, Config::SortModes sortColumns, uint32_t numberBlocks, const INDEX_TYPE* rowColMinMax,
	INDEX_TYPE *rowMaxOperations, uint32_t minimumDensity, bool setSortedBit, uint32_t rowsPerBlock)
{
	cudaFuncSetAttribute(spGEMMNumericLauncher<INDEX_TYPE, GlobalHashMap, GlobalRowOffsetMap, SHARED_HASH_SIZE, SUPPORT_GLOBAL, THREADS/2>, 
		cudaFuncAttributeMaxDynamicSharedMemorySize, sharedMem);
	spGEMMNumericLauncher<INDEX_TYPE, GlobalHashMap, GlobalRowOffsetMap, SHARED_HASH_SIZE, SUPPORT_GLOBAL, THREADS><<<gridDim, blockDim, sharedMem, stream>>>(
